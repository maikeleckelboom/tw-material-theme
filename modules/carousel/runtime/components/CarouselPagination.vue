...
const offsets = computed<number[]>(() =>
  unref(someSlidesDifferentWidth)
    ? unref(props.elements).map(
        (slide) => Number((unref(slide) as HTMLElement).offsetLeft) * -1
      )
    : []
);
...
const setSomeSlidesDifferentWidth = () => {
  someSlidesDifferentWidth.value = unref(props.elements).some((slide) => {
    ...
    const { width: firstSlideWidth } =
      unref(firstSlide).getBoundingClientRect();
...
whenever(someSlidesDifferentWidth, () =>
  unref(props.elements).forEach(provideOffsetLeftProperty)
);
...
const getClosestOffset = (x: number, delta: 1 | -1): number => {
  ...
  const totalNegativeSlides = delta === 1 ? floor(total) : ceil(total);
  return -totalNegativeSlides * (width + spacing);
};

const getClosestSnapOffset = (x: number, delta: 1 | -1): number => {
  const filterOffsets = (offset: number) =>
    Boolean(delta === -1) ? offset > x : offset < x;
...
const scrollTo = (delta: 1 | -1): void => {
  ...
  const closestOffset =
    x === clampedX ? findClosestOffset(clampedX, delta) : clampedX;
...
const prevDisabled = computed(
  () => unref(props.x) >= unref(props.bounds).max! - 10
);
const nextDisabled = computed(
  () => unref(props.x) <= unref(props.bounds).min! + 10
);
