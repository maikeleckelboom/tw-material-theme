...
async function fetchRandomNamedColor() {
  ...
  const response = await fetch(
    `https://www.thecolorapi.com/id?format=json&hex=${randomHex.replace(
      "#",
      ""
    )}`
  );
...
<template>
  <div>
    <div class="mb-6 flex flex-col">
      <h2 class="mt-1 text-xl mb-1.5">Extended Colors</h2>
      <p class="text-sm leading-tight text-on-surface-variant">
        Input a custom color that automatically gets assigned a set of
        complementary tones.
      </p>
    </div>
