...
const isDeeperThanOneLevel = computed<boolean>(() => false)

const transitionName = computed<string>(() => {
  return state.opened ? 'slide-in-right' : 'slide-out-right'
})

const isModal = computed<boolean>(() => false)
</script>
...
<template>
  <Transition :name="transitionName">
    <aside v-if="state.opened"
      ...
      <header class="pl-[16px] pr-[16px] py-[24px] grid grid-cols-[auto,1fr,auto] gap-[12px]"
              data-name="side-sheet-header">
        <div v-if="isDeeperThanOneLevel"
      ...
      <main
          class="flex flex-1 flex-col overflow-y-scroll scrollbar-thin scrollbar-thumb-surface-variant scrollbar-track-surface/50"
