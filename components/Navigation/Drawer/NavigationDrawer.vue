...
<template>
  <div :class="classList"
    ...
    <header class="flex items-center justify-between pl-[28px] pr-[18px] pb-[16px] h-[64px]">
      <span class="text-[14px] text-on-surface-variant font-[500]">
        Yoot Portaal
      </span>
      <button class="flex items-center justify-center rounded-full h-[40px] w-[40px] hover:bg-surface-level-1">
        <Icon :name="`ic:baseline-menu${ isOpened ? '-open' : ''}`"
              class="w-[24px] h-[24px] text-on-surface-variant"
              @click="open"
        />
      </button>
    </header>
