...
type Badge = string | number | { value: string | number; label: string }
...
const getIconName = (icon: string | string[]) => {
  if (Array.isArray(icon)) {
    return active.value && icon.length > 1 ? icon.at(1)! : icon.at(0)!;
  }
  return icon!;
};
</script>
    ...
    <slot>
			<span v-ripple="containerQueryActive" :class="container()">
				<slot name="icon" v-bind="{ icon }">
					<Icon v-if="icon" :name="getIconName(icon)" class="relative z-40 h-4 w-4" />
				</slot>
				...
				<Badge v-if="badge" :class="badgeV()" :style="{ opacity }">
					<slot name="badge">
            {{ isObject(badge) && badge.value ? badge.value : badge }}
					</slot>
				</Badge>
			</span>
    </slot>
