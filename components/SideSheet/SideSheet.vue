...
const onPressed = ({
  ...
  const onMove = (moveEvent: PointerEvent) => {
    ...
    context.transform.x.value = clamp(
      ...
      context.position === "left" ? deltaX * -1 : deltaX
    );
  };
...
useEventListener(currentElement, "pointerdown", onPressed, {
  passive: true
});

const height = computed(() => {
  const { value, min, max } = context.transform.x;
  const percentage = clamp(0, 0.2, (value - min) / (max - min));
  return `calc(100dvh - ${percentage * 20}px)`;
});

const top = computed(() => {
  const { value, min, max } = context.transform.x;
  const percentage = clamp(0, 0.2, (value - min) / (max - min));
  return `${percentage * 10}px`;
});

const transform = computed(
  () => `translateX(${context.transform.x.value}px)`
);
